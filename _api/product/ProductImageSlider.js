    ...
    <div className="bg-white dark:bg-zinc-700 p-4 rounded-lg w-full md:w-1/2 lg:w-1/4">
      ...
      >
        {productImages.map((image, index) => (
          <SwiperSlide
            key={index}
            className="flex items-stretch justify-center cursor-zoom-in relative h-auto"
          >
      ...
      <Swiper
        ...
        modules={[Thumbs]}
        className="mySwiper mt-2 select-none swiper-thumbs"
      >
        {productImages?.map((image, index) => (
          <SwiperSlide
            key={index}
          ...
          >
            <img
              ...
              className="rounded-[5px] object-cover thumbnail-image"
            />
          </SwiperSlide>
        ))}
      </Swiper>
